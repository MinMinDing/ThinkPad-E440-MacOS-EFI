 ...
 *
 * Disassembly of /Volumes/EFI/EFI/CLOVER/ACPI/patched/DSDT.aml, Sat May 12 09:27:53 2018
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x000107DF (67551)
 *     Revision         0x01 **** 32-bit table (V1), no 64-bit math support
 *     Checksum         0x53
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "TP-J9   "
 *     OEM Revision     0x00002270 (8816)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20110316 (537985814)
 */
DefinitionBlock ("", "DSDT", 1, "LENOVO", "TP-J9   ", 0x00002270)
{
    /*
    ...
    OperationRegion (OEMN, SystemMemory, 0xDCF6AF98, 0x14)
    ...
    Scope (_SB)
    {
        ...
        Device (PCI0)
        {
            ...
            Device (LPC)
            {
                ...
                Device (EC)
                {
                    ...
                    Method (LPMD, 0, NotSerialized)
                    {
                        ...
                        Store (Zero, Local2)
                        Store (Zero, Local4)
                        If (LOr (LEqual (OSID, Zero), LEqual (OSID, One)))
                        {
                            Store (0x5A, Local4)
                            If (LEqual (HWAT, 0x2D))
                            {
                    ...
                    Method (GPON, 1, NotSerialized)
                    {
                        If (ISOP ())
                        {
                            If (DGOS)
                            {
                                ...
                                While (LEqual (LCHK (One), Zero))
                                {
                                    Store (0x20, Local0)
                                    While (Local0)
                                    {
                                        If (LEqual (LCHK (One), Zero))
                                        {
                                            Stall (0x64)
                                            Decrement (Local0)
                                        }
            ...
            Device (EXP4)
            {
                ...
                Method (_PSW, 1, NotSerialized)  // _PSW: Power State Wake
                {
                    If (Arg0)
                    {
                        Store (One, ^^LPC.EC.HWPM)
                        Store (One, ^^LPC.EC.HWLU)
                    }
                    Else
                    {
                        Store (Zero, ^^LPC.EC.HWPM)
                        Store (Zero, ^^LPC.EC.HWLU)
                    }
                }
            ...
            Device (XHC)
            {
                ...
                Method (POSC, 3, Serialized)
                {
                    ...
                    If (LAnd (LNot (And (CDW1, One)), LOr (LEqual (\XHC, 0x02), LEqual (\XHC, 0x03))))
                    {
                        SXHC ()
                    }
        ...
        Scope (PCI0.PEG.VID)
        {
            OperationRegion (PEGA, PCI_Config, Zero, 0x04)
            ...
            Method (NVGC, 4, Serialized)
            {
                ...
                While (One)
                {
                    ...
                    ElseIf (LEqual (_T_0, 0x03))
                    {
                        ...
                        CreateField (JTPC, 0x03, One, PWOK)
        ...
        If (LEqual (Arg0, 0x03))
        {
            ...
            If (LOr (USBR, \_SB.PCI0.XHC.XRST))
            {
                If (LOr (LEqual (XHC, 0x02), LEqual (XHC, 0x03)))
                {
                    ...
                    If (LAnd (LEqual (WIN8, Zero), WIN7))
                    {
                        And (\_SB.PCI0.XHC.PR2, 0xFFFFFFEF, \_SB.PCI0.XHC.PR2)
                    }
    ...
    Scope (_SI)
    {
        Method (_SST, 1, NotSerialized)  // _SST: System Status
        {
            ...
            If (LGreaterEqual (Arg0, 0x04))
            {
                Store (Zero, \_SB.PCI0.LPC.EC.HWLO)
            }
        }
    }
